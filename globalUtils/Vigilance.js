    ...
    @wrap((clazz) => {
        ...
        clazz.prototype.initialize = instance => {
            ...
            const config = new JavaAdapter(
                VigilantClass, {},
                new java.io.File(`./config/ChatTriggers/modules/EngineerModules/features/${moduleName}/config/configFile.toml`),
                guiTitle,
                collector,
                sortingBehavior
            );
            ...
            instance.hideProperty = (propertyNameList) => {
                if (typeof propertyNameList === "string") return findProp(propertyNameList).attributesExt.hidden = true;
                propertyNameList.forEach(t => findProp(t).attributesExt.hidden = true);
            }
            instance.showProperty = (propertyNameList) => {
                if (typeof propertyNameList === "string") return findProp(propertyNameList).attributesExt.hidden = false;
                propertyNameList.forEach(t => findProp(t).attributesExt.hidden = false);
            }
            instance.hidePropertyIf = (propertyName, condition) => {
                if (typeof condition === "function") {
                    if (condition()) {
                        if (Array.isArray(propertyName)) return propertyName.forEach(t => instance.hideProperty(t));
...
register("gameUnload", () => configs.forEach(config => {config.markDirty();config.writeData()}));
